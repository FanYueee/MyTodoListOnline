...
// 正式環境預設使用專案內 data/；可透過環境變數換成掛載磁碟或隔離測試目錄。
...
export async function insertTodo(id, todo, afterId) {
  return withLock(id, async () => {
    ...
    // __new__ 是前端初始空白列的保留 id，不能成為永久待辦 id；否則重新
    // 載入時會和新的空白列衝突。保險起見，伺服器端也一律換成 UUID。
    const todoId = todo.id && todo.id !== "__new__" ? todo.id : randomUUID();
    const existing = list.todos.find((t) => t.id === todoId);
    if (existing) {
      existing.text = text;
    } else {
      const clean = {
